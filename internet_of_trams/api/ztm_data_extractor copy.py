...
#         for key, item in lines_and_destinations.items():
#             id = key
            
#             name = list(item.keys())[-1]
            
#             for key, item in sorted(list(item.values())[-1].items(), key=lambda x: int(x[0])):
...
class ZtmDataExtractor:
    def __init__(self, api_key):
        self.connector = ZtmConnector(api_key)
